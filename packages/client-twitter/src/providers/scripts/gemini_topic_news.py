...
def get_topic_news(topics: List[str]) -> List[str]:
    ...
    prompt = f"""Provide the a summary for {topics}. Include any recent news or developments where applicable.

    Format as bullet points with category headers and include specific details.

    Focus only on verified sources.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Get news for specified topics')
    parser.add_argument('--topics', type=str, required=True,
                       help='Comma-separated list of topics (e.g., "Anime,Manga,Games")')

    args = parser.parse_args()
    topics = [topic.strip() for topic in args.topics.split(',')]

    try:
        insights = get_topic_news(topics)
