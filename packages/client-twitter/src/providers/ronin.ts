...
async function executeAnalytics(): Promise<string> {
    return new Promise((resolve, reject) => {
        ...
        const pythonScript = join(__dirname, '..', '..', '..', 'packages', 'client-twitter', 'src', 'providers', 'ronin_analytics.py');
...
async function updateAnalytics() {
    try {
        ...
        console.error('Error updating Ronin analytics:', error);
    }
}
...
const roninProvider: Provider = {
    get: async (runtime: IAgentRuntime, message: Memory, state?: State) => {
        ...
        return analyticsCache.data || 'No Ronin network data available at the moment.';
    },
};

export { roninProvider };